...
public class FileUtils {
    ...
    public static List<File> list2(File dir, List<File> fs) {
        listFile2(dir, fs);
        File[] all = dir.listFiles();
        // 递归获得当前目录的所有子目录
        for (int i = 0; i < all.length; i++) {
     ...
     * @return
     */
    private static List<File> listFile(File dir, List<File> fs, int type) {
        File[] all = dir.listFiles();
        for (int i = 0; i < all.length; i++) {
            File d = all[i];
            ...
            if (type == 0) {
                ...
                if (prefix.equals("log") || prefix.equals("json")) {
                    fs.add(d);
                }
            }
    ...
    public static List<File> listFile2(File dir, List<File> fs) {
        File[] all = dir.listFiles();
        for (int i = 0; i < all.length; i++) {
            File d = all[i];
            if (d.isDirectory()) {
    ...
    public static String readFileFromSd(String DEFAULT_FILENAME){
        String result = null;
        try {
            ...
            br.close();
            result = sb.toString();
            Log.i("LogUtils ","token = "+result);
        } catch (Exception e) {
            Log.i("LogUtils ","get token error e = "+e.getMessage());
            e.printStackTrace();
        }
        return result;
    }
